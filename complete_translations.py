...
# Read the existing good part of translations
with open('/home/user/webapp/lib/translations.ts', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find where to insert new translations
en_section_end = -1
hu_section_end = -1
tr_section_start = -1
...
for i, line in enumerate(lines):
    if 'tr: {' in line:
        tr_section_start = i
        break
    if '  },' in line and en_section_end == -1:
        en_section_end = i

# Read additions from the additions file
additions = """
    // ETICS Facade Page
    eticsFacade: {
      hero: {
...
# Insert before the closing of en section
insertion_point = -1
for i, line in enumerate(lines):
    if line.strip() == '},':  # Find the closing of a major section
        if i > 600 and i < 650:  # The en section should close around here
            insertion_point = i
            break

if insertion_point > 0:
    # Insert the new translations for English
    lines.insert(insertion_point, additions)

# Write the updated content
with open('/home/user/webapp/lib/translations_complete.ts', 'w', encoding='utf-8') as f:
    f.writelines(lines)

print("Complete translations file created!")